  ...
  return (
    <div className="min-h-screen bg-bg-0">
      ...
      <main className="max-w-[1400px] mx-auto px-4 py-6">
        <div className="mb-4">
          <h1 className="text-xl font-bold text-text-primary">생산 실적</h1>
          <p className="text-sm text-text-tertiary mt-0.5">생산 RAW-DATA를 업로드하면 품목류 단위로 재고가 보충됩니다(판매 차감과 상계).</p>
        </div>
        <div className="flex gap-1 mb-5 border-b border-border-primary">
          {tabs.map((t) => (
            <button key={t} onClick={() => setTab(t)}
              className={`px-4 py-2.5 text-sm font-semibold border-b-2 -mb-px transition-colors ${tab === t ? 'border-brand text-accent' : 'border-transparent text-text-tertiary hover:text-text-secondary'}`}>
              {t}
            </button>
          ))}
        </div>
    ...
    <div className="space-y-5 relative">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex bg-bg-1 border border-border-primary rounded-lg p-0.5">
          <button onClick={() => setGran('month')} className={`${C.btn} px-2.5 py-1 ${gran === 'month' ? C.btnPrimary : 'text-text-tertiary'}`}>월</button>
          <button onClick={() => setGran('week')} className={`${C.btn} px-2.5 py-1 ${gran === 'week' ? C.btnPrimary : 'text-text-tertiary'}`}>주</button>
          <button onClick={() => setGran('day')} className={`${C.btn} px-2.5 py-1 ${gran === 'day' ? C.btnPrimary : 'text-text-tertiary'}`}>일</button>
        </div>
        {loading && <span className="text-xs text-text-quaternary">불러오는 중…</span>}
      </div>
      {d && d.record_count === 0 && (
        <div className={`${C.card} p-8 text-center text-sm text-text-quaternary`}>이 기간에 생산 데이터가 없습니다. [업로드] 또는 [실적 입력] 탭에서 추가하세요.</div>
      )}
      {d && d.record_count > 0 && (
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3">
            ...
            <StatCard label="총 원가" value={wonShort(d.total_cost)} tone="text-warning" sub={`원가율 ${d.cost_ratio}%`} />
            <StatCard label={`노무비 (시급 ${fmt(d.hourly_wage)}·야1.5)`} value={wonShort(d.total_labor)} tone="text-cyan" sub={`노무비율 ${d.labor_ratio}%`} />
            <StatCard label="채산성" value={`${d.profitability}배`} tone={d.profitability >= 3 ? 'text-success-light' : 'text-warning'} sub="생산액÷노무비" />
            ...
            {d.by_location.filter((l) => l.location && l.location !== '미상').map((l, i) => (
              <div key={l.location} className={`${C.card} p-4`}>
                ...
                <div className="grid grid-cols-3 gap-2">
                  <div><div className="text-[11px] text-text-tertiary">생산량</div><div className="text-lg font-bold tabular-nums text-text-primary">{numShort(l.qty)}</div></div>
                  <div><div className="text-[11px] text-text-tertiary">투여시간</div><div className="text-lg font-bold tabular-nums text-text-primary">{numShort(l.hours)}h</div></div>
                  <div><div className="text-[11px] text-text-tertiary">시간당생산량</div><div className="text-lg font-bold tabular-nums text-success-light">{fmt(l.hourly_qty)}</div></div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className={`${C.card} p-4 border-l-2 border-l-warning`}>
            <div className="flex items-center gap-2 mb-1">
              <div className="text-sm font-semibold text-text-primary">노무시간·노무비 대조 (근태 연동 · 생산팀 · {granLabel})</div>
              {laborLoading && <span className="text-xs text-text-quaternary">근태 불러오는 중…</span>}
            </div>
            {labor?.error && <div className="text-xs text-danger mb-2">연동 오류: {labor.error}</div>}
            {labor && !labor.error && (
              <>
                <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-3">
                  <StatCard label="생산일보 투여시간" value={`${numShort(labor.total_prod_hours)}h`} />
                  <StatCard label="근태 노무시간(생산팀)" value={`${numShort(labor.total_att_hours)}h`} tone="text-cyan" sub={`정규직 ${numShort(labor.total_regular_hours)}h + 파견 ${numShort(labor.total_dispatch_hours)}h`} />
                  <StatCard label="시간 비율(생산÷근태)" value={`${labor.total_hours_ratio}배`} tone={labor.total_hours_ratio > 1.2 || labor.total_hours_ratio < 0.8 ? 'text-warning' : 'text-success-light'} sub="1에 가까울수록 정합" />
                  <StatCard label="산출 노무비(15k기준)" value={wonShort(labor.total_prod_labor)} tone="text-cyan" />
                ...
                <ResponsiveContainer width="100%" height={240}>
                  <ComposedChart data={labor.series}>
                    ...
                    <YAxis yAxisId="r" orientation="right" tick={{ fill: 'var(--color-text-tertiary)', fontSize: 10 }} domain={[0, 2]} />
                    ...
                    <Legend />
                    <Bar yAxisId="l" dataKey="prod_hours" name="생산일보 투여시간" fill="var(--color-brand-bg)" />
                    <Bar yAxisId="l" dataKey="att_hours" name="근태 노무시간" fill="var(--color-cyan)" />
                    <Line yAxisId="r" type="monotone" dataKey="hours_ratio" name="비율(배)" stroke="var(--color-warning)" strokeWidth={2} dot={false} />
                  </ComposedChart>
                </ResponsiveContainer>
                <div className="text-xs text-text-quaternary mt-2">{labor.note}</div>
              </>
            )}
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className={`${C.card} p-4`}>
              <div className="text-sm font-semibold text-text-primary mb-3">{granLabel} 생산성 (시간당 생산량){cat && ` · ${cat}`}</div>
              <ResponsiveContainer width="100%" height={240}>
                <ComposedChart data={ts}>
                  ...
                  <Tooltip contentStyle={{ background: 'var(--color-bg-level-1)', border: '1px solid var(--color-border-primary)', borderRadius: 8 }} formatter={(v: any) => fmt(v)} />
                  <Bar yAxisId="l" dataKey="hours" name="투여시간" fill="var(--color-border-primary)" />
                  <Line yAxisId="r" type="monotone" dataKey="hourly_qty" name="시간당 생산량" stroke="var(--color-brand-bg)" strokeWidth={2} dot={false} />
                </ComposedChart>
              </ResponsiveContainer>
            </div>
            <div className={`${C.card} p-4`}>
              <div className="text-sm font-semibold text-text-primary mb-3">{granLabel} 채산성 (생산액÷노무비){cat && ` · ${cat}`}</div>
              <ResponsiveContainer width="100%" height={240}>
                <LineChart data={ts}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--color-bg-inset)" />
                  <XAxis dataKey="period" tick={{ fill: 'var(--color-text-tertiary)', fontSize: 10 }} />
                  <YAxis tick={{ fill: 'var(--color-text-tertiary)', fontSize: 10 }} />
                  <Tooltip contentStyle={{ background: 'var(--color-bg-level-1)', border: '1px solid var(--color-border-primary)', borderRadius: 8 }} formatter={(v: any) => v} />
                  <Line type="monotone" dataKey="profitability" name="채산성(배)" stroke="var(--color-success-light)" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className={`${C.card} p-4`}>
              <div className="text-sm font-semibold text-text-primary mb-3">{granLabel} 생산량 · 원가 · 노무비</div>
              <ResponsiveContainer width="100%" height={240}>
                <LineChart data={ts}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--color-bg-inset)" />
                  <XAxis dataKey="period" tick={{ fill: 'var(--color-text-tertiary)', fontSize: 10 }} />
                  ...
                  <YAxis yAxisId="r" orientation="right" tick={{ fill: 'var(--color-text-tertiary)', fontSize: 10 }} />
                  <Tooltip contentStyle={{ background: 'var(--color-bg-level-1)', border: '1px solid var(--color-border-primary)', borderRadius: 8 }} formatter={(v: any) => fmt(v)} />
                  <Line yAxisId="l" type="monotone" dataKey="qty" name="생산량" stroke="var(--color-brand-bg)" strokeWidth={2} dot={false} />
                  <Line yAxisId="r" type="monotone" dataKey="cost" name="원가" stroke="var(--color-warning)" strokeWidth={2} dot={false} />
                  <Line yAxisId="r" type="monotone" dataKey="labor" name="노무비" stroke="var(--color-cyan)" strokeWidth={2} dot={false} />
                </LineChart>
              </ResponsiveContainer>
            </div>
            <div className={`${C.card} p-4`}>
              <div className="text-sm font-semibold text-text-primary mb-3">주간 / 야간 생산량 비교</div>
              <ResponsiveContainer width="100%" height={240}>
                <BarChart data={ts}>
                  <CartesianGrid strokeDasharray="3 3" stroke="var(--color-bg-inset)" />
                  <XAxis dataKey="period" tick={{ fill: 'var(--color-text-tertiary)', fontSize: 10 }} />
                  ...
                  <Tooltip contentStyle={{ background: 'var(--color-bg-level-1)', border: '1px solid var(--color-border-primary)', borderRadius: 8 }} formatter={(v: any) => fmt(v)} />
                  <Legend />
                  <Bar dataKey="day_qty" name="주간" stackId="s" fill="var(--color-brand-bg)" />
                  <Bar dataKey="night_qty" name="야간" stackId="s" fill="var(--color-purple)" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
            <div className={`${C.card} p-4`}>
              <div className="text-sm font-semibold text-text-primary mb-3">기간 누계 품목류별 생산 비중</div>
          ...
          <div className="contents">
            <div className={`${C.card} p-4`}>
              <div className="text-sm font-semibold text-text-primary mb-3">품목류별 생산량</div>
              <ResponsiveContainer width="100%" height={280}>
                <BarChart data={d.by_category.slice(0, 12)} layout="vertical" margin={{ left: 30 }}>
                  ...
                  <YAxis type="category" dataKey="category" tick={{ fill: 'var(--color-text-tertiary)', fontSize: 11 }} width={80} />
                  <Tooltip contentStyle={{ background: 'var(--color-bg-level-1)', border: '1px solid var(--color-border-primary)', borderRadius: 8 }} formatter={(v: any) => fmt(v)} />
            ...
            <div className={`${C.card} p-4`}>
              <div className="flex items-center justify-between mb-3">
                <div className="text-sm font-semibold text-text-primary">생산위치(층)별 · 주야</div>
              </div>
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <div className="text-xs text-text-tertiary mb-2">층별 생산량·시간</div>
                  {d.by_location.map((l, i) => (
                    <div key={l.location} className="flex items-center justify-between py-1.5 border-b border-bg-inset">
                      <span className="text-sm text-text-primary" style={{ color: COLORS[i % COLORS.length] }}>{l.location}</span>
                      <span className="text-sm text-text-secondary">{fmt(l.qty)} · {fmt(l.hours)}h</span>
                    </div>
                  ))}
                </div>
                <div>
                  <div className="text-xs text-text-tertiary mb-2">주간/야간 생산량</div>
                  {d.by_grade.map((g, i) => (
                    <div key={g.grade} className="flex items-center justify-between py-1.5 border-b border-bg-inset">
                      <span className="text-sm text-text-primary">{g.grade}</span>
                      <span className="text-sm text-text-secondary">{fmt(g.qty)}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {d.by_location_shift && d.by_location_shift.length > 0 && (
            <div className={`${C.card} p-4`}>
              <div className="text-sm font-semibold text-text-primary mb-1">생산위치(층)별 주야 · 평균 일 투여시간</div>
              <p className="text-xs text-text-quaternary mb-3">작업일수 대비 총 투여시간(man-hour) 기준 일평균. 층×주야 근무 강도·생산성 비교.</p>
                ...
                <table className="w-full">
                  ...
                  <tbody>
                    {d.by_location_shift.filter((x) => x.location && x.location !== '미상').map((x, i) => (
                      <tr key={`${x.location}-${x.shift}`} className="border-b border-bg-inset">
                        <td className={`${C.td} font-medium text-text-primary`}>{x.location}</td>
                        <td className={C.td}><span className={x.shift === '야간' ? 'text-purple' : 'text-warning'}>{x.shift}</span></td>
                        <td className={C.td}>{fmt(x.days)}일</td>
                        <td className={C.td}>{fmt(x.hours)}h</td>
                        <td className={`${C.td} font-semibold text-cyan`}>{fmt(x.avg_daily_hours)}h/일</td>
          ...
          <div className={`${C.card} p-4`}>
            <div className="text-sm font-semibold text-text-primary mb-3">품목류별 원가·노무비·생산성</div>
            <div className="overflow-x-auto max-h-[360px]">
              <table className="w-full">
                ...
                <tbody>
                  {d.by_category.map((c) => (
                    <tr key={c.category}>
                      <td className={`${C.td} text-text-primary font-medium`}>{c.category}</td>
                      <td className={C.td}>{fmt(c.qty)}</td>
                      <td className={C.td}>{won(c.amount)}</td>
                      <td className={C.td}>{won(c.cost)}</td>
                      <td className={`${C.td} text-cyan`}>{won(c.labor)}</td>
          ...
          <div className={`${C.card} p-4`}>
            <div className="text-sm font-semibold text-text-primary mb-3">담당자별 생산량 · 시간 · 노무비</div>
            <div className="overflow-x-auto max-h-[300px]">
              <table className="w-full">
                <thead className="sticky top-0 bg-bg-1"><tr><th className={C.th}>담당자</th><th className={C.th}>생산량</th><th className={C.th}>생산액</th><th className={C.th}>시간</th><th className={C.th}>노무비</th></tr></thead>
                <tbody>
                  {d.by_worker.map((w) => (
                    <tr key={w.worker}><td className={`${C.td} text-text-primary`}>{w.worker}</td><td className={C.td}>{fmt(w.qty)}</td><td className={C.td}>{won(w.amount)}</td><td className={C.td}>{fmt(w.hours)}h</td><td className={`${C.td} text-cyan`}>{won(w.labor)}</td></tr>
                  ))}
                </tbody>
              </table>
            </div>
          </div>
          <p className="text-xs text-text-quaternary">노무비 = 시급 {fmt(d.hourly_wage)}원 × 생산투여시간 · 개당 노무비 = 노무비 ÷ 생산량 · {range.start} ~ {range.end}</p>

          <MatrixTable
            title="품목별 월별 생산 합계"
            subtitle="품목(품목명)×월 생산량. 상단 기간필터와 무관하게 연도별 1~12월. 맨 아래 총 합계 · 엑셀."
            firstCol="품목"
            loader={(s, e) => getJSON<any>(`/inventory/production/monthly-matrix?start=${s}&end=${e}`, null)}
            showCategory
          />
        </>
      )}
    ...
    <div className="space-y-4">
      ...
      <div className={`${C.card} overflow-x-auto max-h-[70vh]`}>
        <table className="w-full">
          ...
          <tbody>
            {rows.length === 0 ? <tr><td colSpan={12} className="p-6 text-center text-text-quaternary text-sm">데이터 없음</td></tr> :
              rows.map((r) => (
                <tr key={r.id}>
                  <td className={C.td}>{r.prod_date}</td>
                  <td className={C.td}>{r.worker || '-'}</td>
                  <td className={C.td}>{r.location || '-'}</td>
                  <td className={`${C.td} text-text-primary`}>{r.category}</td>
                  ...
                  <td className={`${C.td} text-cyan`}>{won(r.labor_cost)}</td>
...
function PhoneTab() {
  ...
  return (
    <div className="space-y-4">
      <div className={`${C.card} p-4`}>
        <div className="text-sm font-semibold text-text-primary mb-1">생산 담당자 핸드폰 리스트</div>
        <p className="text-xs text-text-quaternary mb-3">여기 담긴 담당자에게 실적 입력 요청 문자를 발송합니다. (서버 SMS 미설정 시 휴대폰 문자앱이 열립니다)</p>
        <div className="flex flex-wrap items-end gap-2 mb-3">
          ...
          <div><div className="text-xs text-text-tertiary mb-1">핸드폰 *</div><input value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="010-1234-5678" className={`${C.input} w-36`} /></div>
          <div><div className="text-xs text-text-tertiary mb-1">담당 층</div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            ...
            <tbody>
              {rows.length === 0 ? <tr><td colSpan={7} className="p-6 text-center text-text-quaternary text-sm">담당자를 추가하세요</td></tr> :
                rows.map((r) => (
                  <tr key={r.id}>
                    <td className={C.td}><input type="checkbox" checked={sel.has(r.id)} onChange={() => toggle(r.id)} /></td>
                    <td className={`${C.td} text-text-primary`}>{r.name}</td>
                    ...
                    <td className={C.td}>
                      <button onClick={() => setForm({ id: r.id, name: r.name, phone: r.phone, location: r.location || '', role: r.role || '' })} className="text-accent text-xs hover:underline mr-2">수정</button>
                      <button onClick={() => del(r.id)} className="text-danger text-xs hover:underline">삭제</button>
                    </td>
                  </tr>
                ))}
            </tbody>
      ...
      <div className={`${C.card} p-4`}>
        <div className="text-sm font-semibold text-text-primary mb-2">실적 입력 요청 문자</div>
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-text-quaternary">체크한 담당자에게(미선택 시 전체) 발송 · 링크 포함</span>
        </div>
      </div>
    </div>
  );
    ...
    <div className="space-y-4">
      <div className={`${C.card} p-4 ${editId ? 'ring-2 ring-brand/60' : ''}`}>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          ...
          <div><div className="text-xs text-text-tertiary mb-1">생산위치(층)</div>
          ...
          <div><div className="text-xs text-text-tertiary mb-1">주야 구분</div>
          ...
          <div><div className="text-xs text-text-tertiary mb-1">개당 원가 (자동)</div><input readOnly value={f.unit_cost !== '' ? Number(f.unit_cost).toLocaleString('ko-KR') : ''} placeholder="품목명 선택 시 자동" className={`${C.input} w-full bg-bg-2 text-text-tertiary`} /></div>
          <div><div className="text-xs text-text-tertiary mb-1">입고 창고 *</div>
  ...
  return (
    <div className="space-y-4">
      <div className={`${C.card} p-4 space-y-3`}>
        <p className="text-sm text-text-secondary">생산 <b>RAW-DATA</b> 엑셀을 올리면 <b>품목류</b> 기준으로 재고가 보충됩니다. 인식 열: <span className="text-text-tertiary">날짜·담당자·생산위치·품목류·품목명·생산량·단가·생산액·원가 등</span>. 같은 행은 재업로드해도 중복 적재되지 않습니다.</p>
        <div className="flex flex-wrap items-end gap-3">
          <div><div className="text-xs text-text-tertiary mb-1">입고 창고 *</div>
            <select value={whId} onChange={(e) => setWhId(e.target.value ? Number(e.target.value) : '')} className={C.input}>
              <option value="">선택</option>{warehouses.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
            </select></div>
          <div><div className="text-xs text-text-tertiary mb-1">RAW-DATA 엑셀</div>
      ...
      {preview && preview.rows && (
        <div className={`${C.card} p-4`}>
          <div className="text-sm font-semibold text-text-primary mb-2">
            ...
            {preview.matched != null && <span className="text-success-light"> · 매칭 {fmt(preview.matched)}</span>}
            {preview.unmatched != null && preview.unmatched > 0 && <span className="text-danger"> · 미매칭 {fmt(preview.unmatched)}</span>}
          </div>
          {preview.unmatched_keys?.length > 0 && (
            <div className="mb-3 text-xs text-warning">
              ...
              <div className="text-text-quaternary mt-1">→ 품목 관리에서 표준명/별칭을 맞추면 다음 업로드부터 반영됩니다.</div>
            </div>
          )}
      ...
      <div className={`${C.card} p-4`}>
        ...
        {batches.length === 0 ? <div className="text-sm text-text-quaternary">업로드 이력 없음</div> : (
          <div className="overflow-x-auto">
            <table className="w-full">
              ...
              <tbody>
                {batches.map((b) => (
                  <tr key={b.batch_id}>
                    ...
                    <td className={C.td}><button onClick={() => delBatch(b.batch_id)} className="text-danger text-xs hover:underline">삭제</button></td>
                  </tr>
                ))}
              </tbody>
            </table>
