  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] flex flex-col gap-2">
      {toasts.map(t => (
        <div
          key={t.id}
          className={`flex items-center gap-3 px-5 py-3 rounded-lg shadow-lg text-sm font-medium transition-all animate-slide-up
  ...
  const renderEditableCell = (plan: ProductionPlan, field: keyof ProductionPlan, rowIdx: number) => {
    ...
    if (isAutoCalc) {
      return (
        <td
          key={field}
          className={`px-2 py-1.5 border border-gray-200 bg-gray-50 text-right text-sm tabular-nums select-none`}
        >
          {displayValue}
        </td>
      );
    ...
    return (
      <td
        key={field}
        className={`px-2 py-1.5 border border-gray-200 text-sm cursor-pointer hover:bg-blue-50 transition-colors truncate max-w-[200px]
          ${isRightAlign ? 'text-right tabular-nums' : ''}
          ${plan._dirty ? 'bg-amber-50/40' : ''}
        `}
      ...
      <main className="max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">생산일보 RAW-DATA 기반 생산 계획 관리</p>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-gradient-to-r from-violet-500 to-purple-600 text-white text-sm font-medium shadow-sm hover:from-violet-600 hover:to-purple-700 disabled:opacity-60 disabled:cursor-not-allowed transition-all"
            >
              {bulkAiLoading ? (
                <>
                  <LoadingSpinner />
            ...
            <button
              ...
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-blue-600 text-white text-sm font-medium shadow-sm hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
            ...
            <button
              onClick={addNewPlan}
              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-600 text-white text-sm font-medium shadow-sm hover:bg-emerald-700 transition-colors"
            >
              <PlusIcon />
              <span>계획 추가</span>
            </button>
            ...
            <button
              onClick={exportCsv}
              className="flex items-center gap-2 px-3 py-2 rounded-lg border border-gray-300 bg-white text-gray-700 text-sm font-medium hover:bg-gray-50 transition-colors"
            >
              <DownloadIcon />
              <span>CSV</span>
            </button>
            ...
            {alertsExpanded && (
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {[...urgentAlerts, ...warningAlerts].map(alert => (
                  ...
                  >
                    <div className={`mt-0.5 ${alert.level === 'urgent' ? 'text-red-500' : 'text-amber-500'}`}>
                      <ExclamationIcon />
                    </div>
                    <div className="flex-1 min-w-0">
                      <p className={`text-sm font-semibold ${alert.level === 'urgent' ? 'text-red-800' : 'text-amber-800'}`}>
                        {alert.title}
                      </p>
                      <p className={`text-xs mt-0.5 ${alert.level === 'urgent' ? 'text-red-600' : 'text-amber-600'}`}>
                        {alert.description}
                      </p>
                      {alert.due_date && (
                        <p className="text-xs text-gray-500 mt-1">납기: {alert.due_date}</p>
                      )}
                    </div>
                  </div>
                ))}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
            <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">총 생산량</p>
            <p className="text-2xl font-bold text-gray-900 mt-1">{fmtNum(summary.totalQty)}</p>
            <p className="text-xs text-gray-400 mt-1">{plans.length}개 품목</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
            <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">총 생산액</p>
            <p className="text-2xl font-bold text-blue-600 mt-1">{fmtNum(summary.totalValue)}</p>
            <p className="text-xs text-gray-400 mt-1">원</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
            <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">총 투여 시간</p>
            <p className="text-2xl font-bold text-emerald-600 mt-1">{fmtNumDec(summary.totalHours)}</p>
            <p className="text-xs text-gray-400 mt-1">시간</p>
          </div>
          <div className="bg-white rounded-xl border border-gray-200 p-4 shadow-sm">
            <p className="text-xs text-gray-500 font-medium uppercase tracking-wide">AI 추천 대기</p>
        ...
        <div
          ...
          tabIndex={0}
          className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-auto focus:outline-none focus:ring-2 focus:ring-blue-300"
            ...
            <table className="w-full border-collapse text-sm">
              ...
              <thead className="sticky top-0 z-10">
                <tr className="bg-gray-100 border-b-2 border-gray-300">
                  <th className="px-2 py-2.5 border border-gray-200 text-center text-xs font-semibold text-gray-600 w-8 bg-gray-100">
                    <input
                      type="checkbox"
                  ...
                  <th className="px-2 py-2.5 border border-gray-200 text-center text-xs font-semibold text-gray-600 w-10 bg-gray-100">
                    No
                  </th>
                  {COLUMN_HEADERS.map((header, i) => (
                    <th
                      key={i}
                      className={`px-2 py-2.5 border border-gray-200 text-xs font-semibold text-gray-600 whitespace-nowrap bg-gray-100 ${
                  ...
                  <th className="px-2 py-2.5 border border-gray-200 text-center text-xs font-semibold text-gray-600 w-16 bg-gray-100">
                    삭제
                  </th>
                </tr>
              </thead>
              <tbody>
                {plans.length === 0 ? (
                  <tr>
                    <td colSpan={COLUMN_KEYS.length + 3} className="text-center py-16 text-gray-400">
                  ...
                  <>
                    {plans.map((plan, rowIdx) => (
                      <tr
                        ...
                        className={`hover:bg-blue-50/40 transition-colors ${
                          plan._dirty ? 'bg-amber-50/20' : rowIdx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'
                        } ${selectedRows.has(plan.id) ? '!bg-blue-50' : ''}`}
                      >
                        {/* Checkbox */}
                        ...
                        {/* Row number */}
                        <td className="px-2 py-1.5 border border-gray-200 text-center text-xs text-gray-400 tabular-nums">
                          {rowIdx + 1}
                        </td>
                        {/* Data cells */}
                    ...
                    {/* ── Totals Row ──────────────────────────────────────── */}
                    <tr className="bg-gray-100 border-t-2 border-gray-300 font-semibold sticky bottom-0">
                      <td className="px-2 py-2 border border-gray-200"></td>
                      <td className="px-2 py-2 border border-gray-200"></td>
                      {/* date */}
                      <td className="px-2 py-2 border border-gray-200 text-center text-xs text-gray-600">합계</td>
                      {/* manager */}
                      <td className="px-2 py-2 border border-gray-200"></td>
                      {/* location */}
                      <td className="px-2 py-2 border border-gray-200"></td>
                      ...
                      <td className="px-2 py-2 border border-gray-200"></td>
                      {/* product_name */}
                      <td className="px-2 py-2 border border-gray-200 text-xs text-gray-500 text-center">{plans.length}개 품목</td>
                      {/* quantity */}
                      <td className="px-2 py-2 border border-gray-200 text-right text-xs text-gray-900 tabular-nums">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold text-gray-900">생산 계획 공유</h3>
              <button onClick={() => { setShowShareModal(false); setShareSuccess(false); setShareEmail(''); }} className="text-gray-400 hover:text-gray-600">
                <XIcon />
              </button>
            </div>
            ...
            {shareSuccess ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-700 font-medium">공유가 완료되었습니다</p>
              </div>
            ) : (
              <>
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-1.5">공유 방법</label>
                ...
                <button
                  ...
                  className="w-full py-2.5 rounded-lg bg-blue-600 text-white text-sm font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-colors"
                >
