    ...
    <div className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
      <div
        className={`max-w-full lg:max-w-5xl rounded-2xl p-4 ${
          isUser
            ? 'bg-gradient-to-r from-brand to-brand text-white shadow-[0px_7px_32px_rgba(0,0,0,0.35)]'
            : 'bg-bg-1 border border-border-primary shadow-[0px_3px_12px_rgba(0,0,0,0.2)]'
        }`}
      >
        {/* 메시지 내용 */}
        ...
        {!isUser && message.columns && message.rows && message.rows.length > 0 && (
          <div className="mt-3">
              ...
              <div className="flex gap-1.5">
                <button
                  ...
                  className={`text-xs px-2.5 py-1 rounded-lg transition-colors flex items-center gap-1 ${
                    showChart ? 'bg-brand/15 text-accent' : 'text-text-tertiary hover:text-link hover:bg-brand/10'
                  }`}
                >
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <button
                  ...
                  className="text-xs px-2.5 py-1 text-text-tertiary hover:text-success hover:bg-success/10 rounded-lg transition-colors flex items-center gap-1"
                >
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            {showChart ? (
              <div className="border border-border-primary rounded-xl overflow-hidden bg-bg-1 p-3">
        ...
        {!isUser && message.content.length > 200 && (
          <div className="mt-3 pt-3 border-t border-border-primary">
            <button
              ...
              className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-link hover:text-accent bg-brand/10 hover:bg-brand/15 rounded-lg transition-colors"
            >
        ...
        {/* 타임스탬프 */}
        <p className={`text-[10px] mt-2 flex items-center gap-1 ${isUser ? 'text-link/60' : 'text-text-quaternary'}`}>
