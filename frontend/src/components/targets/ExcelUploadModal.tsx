      ...
      <div className="relative bg-white rounded-2xl shadow-xl w-full max-w-6xl max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
          <h2 className="text-lg font-semibold text-slate-800">
            엑셀 업로드
            {step === 2 && <span className="ml-2 text-sm font-normal text-slate-500">- 미리보기</span>}
          </h2>
          <button
            onClick={handleClose}
        ...
        <div className="flex-1 overflow-auto p-6">
          ...
          {successCount !== null ? (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <h3 className="text-lg font-semibold text-slate-800 mb-2">업로드 완료</h3>
              <p className="text-slate-600 mb-6">
                총 <span className="font-bold text-blue-600">{successCount}건</span>의 목표 데이터가 등록되었습니다.
              </p>
              <button
              ...
              >
                ...
                {file ? (
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-2">클릭하여 다른 파일 선택</p>
                  </div>
                ) : (
                  <div>
                    ...
                    <p className="text-xs text-slate-400 mt-1">.xlsx, .xls 파일만 지원</p>
                  </div>
                )}
              </div>
                  ...
                  <table className="w-full text-sm">
                    <thead>
                      <tr className="bg-slate-100 border-b border-slate-200">
                        <th className="px-3 py-2 text-left font-medium text-slate-600 whitespace-nowrap sticky left-0 bg-slate-100 z-10">담당자</th>
                        <th className="px-3 py-2 text-left font-medium text-slate-600 whitespace-nowrap">구분</th>
                        <th className="px-3 py-2 text-left font-medium text-slate-600 whitespace-nowrap">판매채널</th>
                        {monthLabels.map((m) => (
                          <th key={m} className="px-3 py-2 text-right font-medium text-slate-600 whitespace-nowrap">{m}</th>
                        ))}
                      </tr>
                    </thead>
                    <tbody>
...
function ManagerGroup({
  ...
  return (
    <>
      {rows.map((row, idx) => (
        <tr key={`${manager}-${idx}`} className="border-b border-slate-100 hover:bg-slate-50">
          {idx === 0 ? (
            <td
              className="px-3 py-2 font-medium text-slate-800 whitespace-nowrap sticky left-0 bg-white z-10"
              rowSpan={rows.length}
            >
              {manager}
            </td>
          ) : null}
          <td className="px-3 py-2 text-slate-600 whitespace-nowrap">{row.구분}</td>
          <td className="px-3 py-2 text-slate-600 whitespace-nowrap">{row.판매채널}</td>
          {row.monthly.map((val, mIdx) => (
            <td key={mIdx} className="px-3 py-2 text-right text-slate-700 whitespace-nowrap tabular-nums">
              {formatNumber(val)}
            </td>
          ))}
        </tr>
      ...
      <tr className="bg-blue-50 border-b border-blue-200">
        <td className="px-3 py-2 font-semibold text-blue-700 whitespace-nowrap sticky left-0 bg-blue-50 z-10">
          {manager} 소계
        </td>
        <td className="px-3 py-2" colSpan={2}></td>
        {monthLabels.map((_, mIdx) => (
          <td key={mIdx} className="px-3 py-2 text-right font-semibold text-blue-700 whitespace-nowrap tabular-nums">
